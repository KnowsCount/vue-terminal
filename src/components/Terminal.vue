	...
	<window>
		<div class="content" ref="content">
			<div v-if="showWelcome">
				<code
					v-if="welcome"
					v-html="$options.filters.linkify(welcome.trim())"
				></code>
				<code v-if="welcome">{{ " " }}</code>
				<code
					>Type <strong>help</strong> to list the available
					commands</code
				>
				<code>输入 <strong>help</strong> 以查看可用指令</code>
			</div>
			<div class="terminal">
				<code
					v-for="(line, index) in lines"
					:key="index"
					v-html="$options.filters.linkify(line)"
				></code>
			</div>
...
export default createComponent({
	...
	filters: {
		...
		linkify(text: string) {
			...
			// will not take http into consideration lol
		},
	},
	setup(props, context) {
		...
		const options = computed(() => {
			return children.value.filter((d) => {
				return (
					d.name !== null && d.name.startsWith(filterBasename.value)
				);
			});
		});
		...
		const filterBasename = computed(() => {
			return state.filter.substring(
				filterBreakpoint.value + 1,
				state.filter.length
			);
		});
		...
		// I tried to do these both at the same time but it didn't work??
